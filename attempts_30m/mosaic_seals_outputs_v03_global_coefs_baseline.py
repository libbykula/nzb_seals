...
for year in [2023]:
    ...
    for i in range(0, 12):        
        ...
        tif_dir = os.path.join(p.user_dir, os.sep.join(p.extra_dirs), p.project_name, 'intermediate/fine_processed_inputs/lulc/mapbiomas/seals7/')
        tif_path = os.path.join(tif_dir, 'lulc_mapbiomas_seals7_' + str(year) + '.tif')
        tif_dict[year].append((tif_path, 1))

    # Make a raster to stitch into 
    output_path = os.path.join(p.user_dir, os.sep.join(p.extra_dirs), 'lulc_mapbiomas_seals7_' + str(year) + '_global_stitched.tif')
