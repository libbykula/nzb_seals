...
for year in [2050]:
    ...
    for i in range(0, 12):        
        ...
        tif_dir = os.path.join(p.user_dir, os.sep.join(p.extra_dirs), p.project_name, 'intermediate/coarse_change/coarse_simplified_ha/ssp5/rcp85/luh2-magpie/bau/' + str(year))
        tif_path = os.path.join(tif_dir, 'forest_ha_ssp5_rcp85_luh2-magpie_bau_' + str(year) + '.tif')
        tif_dict[year].append((tif_path, 1))

    # Make a raster to stitch into 
    output_path = os.path.join(p.user_dir, os.sep.join(p.extra_dirs), 'forest_ha_ssp5_rcp85_luh2-magpie_bau_' + str(year) + '_stitched.tif')
