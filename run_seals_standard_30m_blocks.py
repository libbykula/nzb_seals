...
if __name__ == '__main__':
    # for block in ['466_346', '480_343']: # broken, working
    for block in ['466_346']:       
        ...
        p.project_name = 'Brazil_magpie_mapbiomas_policy6_30m_block_' + block + hb.pretty_time() 

        # p.project_name = p.project_name + '_' # If don't you want to recreate everything each time, comment out this line.

        # Based on the paths above, set the project_dir. All files will be created in this directory.
        p.project_dir = os.path.join(p.user_dir, os.sep.join(p.extra_dirs), p.project_name)

        ## Preparing the AOI tiles 
        os.makedirs(p.project_dir, exist_ok=True)
        os.makedirs(os.path.join(p.project_dir, 'intermediate/project_aoi'), exist_ok=True)
        block_aoi = gpd.read_file(os.path.join(p.ext_drive, 'base_data/cartographic/ee/' + block + '.gpkg'))
        block_aoi.to_file(os.path.join(p.project_dir, 'intermediate/project_aoi/aoi_BRA_clipped_forreal.gpkg'), driver='GPKG') 
        
        p.set_project_dir(p.project_dir)

        p.run_in_parallel = 0 # Must be set before building the task tree if the task tree has parralel iterator tasks.
        p.cython_reporting_level = 11 # not working 
        p.calibration_reporting_level = 11 # not working 
        ...
        p.base_data_dir = os.path.join(p.ext_drive, 'base_data')
        ...
        p.scenario_definitions_filename = 'standard_scenarios_BRA_magpie_mapbiomas_policy6_30m_hb_pyr_near.csv' 
        # p.scenario_definitions_filename = 'standard_scenarios_BRA_magpie_mapbiomas_policy6_30m_3c.csv' 
        ...
        # p.processing_resolution = 1/360 # In degrees. Must be in pyramid_compatible_resolutions
        p.processing_resolution = 0.25 # In degrees. Must be in pyramid_compatible_resolutions
        p.cython_reporting_level = 11 
        p.calibration_reporting_level = 11 
