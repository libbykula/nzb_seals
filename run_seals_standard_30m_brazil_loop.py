...
if __name__ == '__main__':
    ...
    for i in [11]:     
        ...
        p.user_dir = 'C:/Users/kibby'
        p.extra_dirs = ['Files', 'seals', 'projects']
        p.project_name = 'Brazil_magpie_mapbiomas_policy6_30m_3600s_coefs_fixing_1deg_' + str(i)
        # p.project_name = 'Brazil_magpie_mapbiomas_policy6_30m_global_coefs_' + str(i)
        ...
        all_tiles_aoi = gpd.read_file(os.path.join(p.base_data_dir, 'cartographic/ee/brazil_equal_land_tiles.gpkg'))
        chunk_aoi = all_tiles_aoi[all_tiles_aoi['tile_id'] == i]
        chunk_aoi.to_file(os.path.join(p.project_dir, 'intermediate/project_aoi/aoi_BRA_clipped_forreal.gpkg'), driver='GPKG') 
        ...
        p.scenario_definitions_filename = 'standard_scenarios_BRA_magpie_mapbiomas_policy6_30m_3600sec_v04.csv' # no pa
        # p.scenario_definitions_filename = 'standard_scenarios_BRA_magpie_mapbiomas_policy6_30m_global_coefs.csv' 

        # p.scenario_definitions_filename = 'standard_scenarios_BRA_magpie_mapbiomas_policy6_30m_3c.csv' 
        p.scenario_definitions_path = os.path.join(p.base_data_dir, 'seals', 'default_inputs', p.scenario_definitions_filename)
